...
void allocate_shared_memory (int segment_ids [], int *segment_counter)
{
    ...
    {
        printf ("RicartAgrawala: shmget failed for sem\n");
        exit (1);
    }
    if ((sem = (sem_t *) shmat (segment_id, (const void *) NULL, 0)) == (sem_t *) -1)
    {
        printf ("RicartAgrawala: shmat failed for sem\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_server_ready\n");
        exit (1);
    }
    if ((my_server_ready = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_server_ready\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_ticket_no\n");
        exit (1);
    }
    if ((my_ticket_no = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_ticket_no\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_highest_ticket_no\n");
        exit (1);
    }
    if ((my_highest_ticket_no = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_highest_ticket_no\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_request\n");
        exit (1);
    }
    if ((my_request = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_request\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_replies\n");
        exit (1);
    }
    if ((my_replies = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_replies\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_deferred_table\n");
        exit (1);
    }
    if ((my_deferred_table_address = (char *) shmat (segment_id, (const void *) NULL, 0)) == (char *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_deferred_table\n");
        exit (1);
    }
    ...
    if ((segment_id = shmget (segment_key, segment_size, segment_permissions)) == -1)
    {
        printf ("RicartAgrawala: shmget failed for my_deferred_count\n");
        exit (1);
    }
    if ((my_deferred_count = (int *) shmat (segment_id, (const void *) NULL, 0)) == (int *) -1)
    {
        printf ("RicartAgrawala: shmat failed for my_deferred_count\n");
        exit (1);
    }
...
void generate_ftok_parameter (char parameter [])
{
    ...
    {
        printf ("RicartAgrawala: fopen failed for ftokParameterFile read");
        exit (1);
    }
    ...
    if (ftok_parameter_file == (FILE *) NULL)
    {
        printf ("RicartAgrawala: fopen failed for ftokParameterFile write");
        exit (1);
    }
...
void generate_ftok_file_name (char file_name [], char parameter [])
{
    ...
    {
        printf ("RicartAgrawala: fopen failed for %s write", file_name);
        exit (1);
    }
    fprintf (ftok_file, "%s\n", file_name);
    fclose (ftok_file);
...
void initialize_semaphore ()
{
    ...
    {
        perror ("RicartAgrawala: sem_init failed for sem");
        exit (1);
    }
...
void create_client_and_server_threads ()
{
    ...
    {
        perror ("RicartAgrawala: calloc failed to allocate memory for thread IDs");
        exit (1);
    }

    if (error = pthread_create (tids, (const pthread_attr_t *) NULL, server_thread, (void *) NULL))
    {
        printf ("RicartAgrawala: pthread_create failed to create server_thread (%s)\n", strerror (error));
        exit (1);
    }

    if (error = pthread_create (tids + 1, (const pthread_attr_t *) NULL, client_thread, (void *) NULL))
    {
        printf ("RicartAgrawala: pthread_create failed to create client_thread (%s)\n", strerror (error));
        exit (1);
    }
...
void wait_for_threads_to_finish ()
{
    ...
    {
        printf ("RicartAgrawala: pthread_join failed to join server_thread (%s)\n", strerror (error));
        exit (1);
    }
...
void destroy_semaphore ()
{
    ...
    {
        perror ("RicartAgrawala: sem_destroy failed to destroy sem");
        exit (1);
    }
...
void deallocate_shared_memory (int segment_ids [], int *segment_counter)
{
    ...
    for (i = 0; i < *segment_counter; i ++)
    {
        ...
        {
            printf ("RicartAgrawala: shmctl failed to deallocate shared memory for segment %d\n", i);
        }
    }
